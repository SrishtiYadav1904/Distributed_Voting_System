...
@app.route('/')
def index():
    """Root: always show login page first (clear any stale session)."""
    # Clear stale session so the app always starts at login
    session.clear()
...
@app.route('/main')
def main_view():
    """Main voting interface (accessible after successful login)."""
    # Validate session: ensure "logged_in" is present and maps to a real voter
    if 'logged_in' not in session:
        return redirect(url_for('index'))

    # Extra validation: check stored name and voter_id against VOTERS_DB
    name = session.get('name')
    voter_id = session.get('voter_id')
    if not name or not voter_id or not get_voter_by_credentials(name, voter_id):
        # clear any stale session and force login
        session.clear()
        return redirect(url_for('index'))
